...
% it is unavoidable, because the extension sits right of the expert masks so
% anything fed from the U-Net must cross the experts -> D^GT arrow.
...
% the four experts; the one drawn at random as Y_g is outlined in green
...
\emph{one} randomly drawn expert $Y_g$ (green outline), and the sampled $z$
(green block, 6 channels) is concatenated to the last U-Net activation
(32 channels). The three $1{\times}1$ convolutions of $f_{\text{comb}}$ combine
them.
Inside the box, our extension. Note that $F(x)$ is taken \emph{before} $z$ is
added, so $\hat D$ depends on the image alone, and that the $K$ passes behind $U$
each draw a fresh $z_k$ from the \emph{prior}. \textbf{Every thumbnail is
...
draws: 8 of the 16 samples for this crop are empty, though all four experts
